...
const ripples = {
  ...
  hide(el: HTMLElement | null) {
    ...
    const animation: any = ripples[ripples.length - 1];
...
function rippleShow(e: MouseEvent | TouchEvent) {
  ...
  if (!element || !(element as any)._ripple || (element as any)._ripple.touched) return;
  if (isTouchEvent(e)) {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    node.context && node.context.$nextTick(() => {
      ...
      if (computed && computed.display === 'inline') {
        ...
        console.log('v-ripple can only be used on block-level elements', ...context);
      }
    });
  }
}
